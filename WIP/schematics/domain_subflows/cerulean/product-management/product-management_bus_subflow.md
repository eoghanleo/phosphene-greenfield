...
Scope: **only** the `<product-management>` orchestration subflow, driven by **bus commits + PR events**:
...
- Autoscribe may also react to **request signals** addressed to `<product-management>` (bus-native bounce)
...
- Autoscribe also consumes `phosphene.request.<requesting_domain>.product-management.<work_type>.v1` signals and treats them as upstream triggers (idempotent on `parents`).
