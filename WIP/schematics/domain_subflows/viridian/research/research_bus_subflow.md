## `<research>` subflow — bus-only orchestration (lane: `viridian`)

Scope: **only** the `<research>` orchestration subflow, driven by **bus commits + PR events**:
...
- Autoscribe may also react to **request signals** addressed to `<research>` (bus-native bounce)
...
sequenceDiagram
  ...
  participant AS as gantry.autoscribe.research
  participant H as gantry.hopper.research
  participant P as gantry.prism.research
  participant D as gantry.detector.research
  participant K as gantry.condenser.research
  participant T as gantry.trap.research
...
- Autoscribe consumes `phosphene.request.<requesting_domain>.research.<work_type>.v1` signals.
...
#### `phosphene.autoscribe.research.issue_created.v1`

- **Must include**: `work_id`, `domain:"research"`, `issue_number`, `lane:"viridian"`, `parents:[<request_or_merge_signal_id>]`

#### `phosphene.hopper.research.start.v1`

- **Must include**: `work_id`, `issue_number`, `lane:"viridian"`, `parents:[<issue_created_signal_id>]`

#### `phosphene.prism.research.branch_invoked.v1`

- **Must include**: `work_id`, `issue_number`, `lane:"viridian"`, `phos_id`, `parents:[<start_signal_id>]`

#### `phosphene.done.research.receipt.v1`

- **Must include**: `work_id`, `issue_number`, `lane:"viridian"`, `parents:[<branch_invoked_signal_id>]`

#### `phosphene.detector.research.approve.v1`

- **Must include**: `work_id`, `issue_number`, `lane:"viridian"`, `parents:[<done_receipt_signal_id>]`

#### `phosphene.detector.research.trap.v1`
