...
# Minimal programmatic scoring for <ideation> artifacts.
...
usage() {
  cat <<'EOF'
Usage:
  ./.github/scripts/ideation-domain-done-score.sh [--docs-root <dir>] [--file <path>] [--min-score <0..100>] [--quiet]
EOF
}
...
DOCS_ROOT="$ROOT/phosphene/domains/ideation/output"
FILE=""
MIN_SCORE="10"
QUIET=0
...
if [[ -n "${FILE:-}" ]]; then
  ...
  while IFS= read -r f; do
    [[ -n "${f:-}" ]] || continue
    files+=("$f")
  done < <(find "$DOCS_ROOT" -type f -name "IDEA-*.md" 2>/dev/null | sort)
fi
...
out_words="$(cat "${files[@]}" 2>/dev/null \
  ...
  | wc -w | awk '{print $1}')"
score="$(phos_ds_score_minimal "$out_items" "$out_words" 10 50 100)"
