...
# product-architecture-domain-done-score.sh
# Minimal programmatic scoring for <product-architecture> artifacts.
...
usage() {
  cat <<'EOF'
Usage:
  ./.github/scripts/product-architecture-domain-done-score.sh [--docs-root <dir>] [--file <path>] [--min-score <0..100>] [--quiet]
EOF
}
...
DOCS_ROOT="$ROOT/phosphene/domains/product-architecture/output"
FILE=""
MIN_SCORE="10"
QUIET=0
...
if [[ -n "${FILE:-}" ]]; then
  ...
  while IFS= read -r f; do
    [[ -n "${f:-}" ]] || continue
    files+=("$f")
  done < <(find "$DOCS_ROOT" -type f -name "ARCH-*.md" 2>/dev/null | sort)
fi

n_files="${#files[@]}"
if [[ "$n_files" -eq 0 ]]; then
  fail "No ARCH artifacts found under: $DOCS_ROOT"
fi
...
if [[ "$QUIET" -ne 1 ]]; then
  echo "PHOSPHENE — Done Score  <product-architecture>"
  echo "============================================================"
  echo "Result:    ${result}   Overall: ${score}/100   Threshold: ${MIN_SCORE}"
  echo ""
  echo "Inputs:"
  echo "  - architecture files: ${n_files}"
