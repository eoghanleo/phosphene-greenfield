...
# scrum-management-domain-done-score.sh
# Minimal programmatic scoring for <scrum-management> artifacts.
...
usage() {
  cat <<'EOF'
Usage:
  ./.github/scripts/scrum-management-domain-done-score.sh [--docs-root <dir>] [--file <path>] [--min-score <0..100>] [--quiet]
EOF
}
...
DOCS_ROOT="$ROOT/phosphene/domains/scrum-management/output"
FILE=""
MIN_SCORE="10"
QUIET=0
...
if [[ -n "${FILE:-}" ]]; then
  ...
  while IFS= read -r f; do
    [[ -n "${f:-}" ]] || continue
    files+=("$f")
  done < <(find "$DOCS_ROOT" -type f -name "ISSUE-*.md" 2>/dev/null | sort)
fi

n_files="${#files[@]}"
if [[ "$n_files" -eq 0 ]]; then
  fail "No ISSUE artifacts found under: $DOCS_ROOT"
fi
...
if [[ "$QUIET" -ne 1 ]]; then
  echo "PHOSPHENE — Done Score  <scrum-management>"
  echo "============================================================"
  echo "Result:    ${result}   Overall: ${score}/100   Threshold: ${MIN_SCORE}"
  echo ""
  echo "Inputs:"
  echo "  - issue mirror files: ${n_files}"
