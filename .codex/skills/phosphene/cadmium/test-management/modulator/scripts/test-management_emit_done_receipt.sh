#!/usr/bin/env bash
#
# PHOSPHENE <test-management> — emit DONE receipt (JSONL bus)
#
set -euo pipefail
...
parent_line="$(
  grep -F "\"signal_type\":\"phosphene.prism.test-management.branch_invoked.v1\"" "$BUS" \
...
run_marker="$WORK_ID"
output_key="done:receipt:test-management:issue:${ISSUE_NUMBER}"
...
line="{\"signal_version\":1,\"signal_id\":\"${signal_id}\",\"signal_type\":\"phosphene.done.test-management.receipt.v1\",\"work_id\":\"${WORK_ID}\",\"domain\":\"test-management\",\"issue_number\":${ISSUE_NUMBER},\"lane\":\"${lane}\",\"phos_id\":\"${phos_id}\",\"parents\":[\"${parent_signal_id}\"],\"run_marker\":\"${run_marker}\",\"output_key\":\"${output_key}\",\"ok\":true,\"created_utc\":\"${created_utc}\"}"
