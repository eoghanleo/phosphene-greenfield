#!/usr/bin/env bash
#
# PHOSPHENE <product-vision> — emit DONE receipt (JSONL bus)
#
set -euo pipefail
...
parent_line="$(
  grep -F "\"signal_type\":\"phosphene.prism.product-vision.branch_invoked.v1\"" "$BUS" \
...
run_marker="$WORK_ID"
output_key="done:receipt:product-vision:issue:${ISSUE_NUMBER}"
...
line="{\"signal_version\":1,\"signal_id\":\"${signal_id}\",\"signal_type\":\"phosphene.done.product-vision.receipt.v1\",\"work_id\":\"${WORK_ID}\",\"domain\":\"product-vision\",\"issue_number\":${ISSUE_NUMBER},\"lane\":\"beryl\",\"phos_id\":\"${phos_id}\",\"parents\":[\"${parent_signal_id}\"],\"run_marker\":\"${run_marker}\",\"output_key\":\"${output_key}\",\"ok\":true,\"created_utc\":\"${created_utc}\"}"
