#!/usr/bin/env bash
#
# PHOSPHENE <product-architecture> — emit DONE receipt (JSONL bus)
#
set -euo pipefail
...
parent_line="$(
  grep -F "\"signal_type\":\"phosphene.prism.product-architecture.branch_invoked.v1\"" "$BUS" \
...
lane="$(
  printf "%s" "$parent_line" \
    | grep -oE '"lane"[[:space:]]*:[[:space:]]*"[^"]*"' \
    | head -n 1 \
    | sed -E 's/^"lane"[[:space:]]*:[[:space:]]*"//; s/"$//'
)"
lane="${lane:-cerulean}"

created_utc="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
run_marker="$WORK_ID"
output_key="done:receipt:product-architecture:issue:${ISSUE_NUMBER}"
...
line="{\"signal_version\":1,\"signal_id\":\"${signal_id}\",\"signal_type\":\"phosphene.done.product-architecture.receipt.v1\",\"work_id\":\"${WORK_ID}\",\"domain\":\"product-architecture\",\"issue_number\":${ISSUE_NUMBER},\"lane\":\"${lane}\",\"phos_id\":\"${phos_id}\",\"parents\":[\"${parent_signal_id}\"],\"run_marker\":\"${run_marker}\",\"output_key\":\"${output_key}\",\"ok\":true,\"created_utc\":\"${created_utc}\"}"
