...
"$ROOT/virric/domains/research/scripts/validate_research_assessment_bundle.sh" "$BUNDLE_DIR" >/dev/null

echo "--- add reference solution ---"
rs_line="$("$ROOT/virric/domains/research/scripts/add_reference_solution.sh" \
  --bundle "$BUNDLE_DIR" \
...
e_line="$("$ROOT/virric/domains/research/scripts/add_evidence_record.sh" \
  --bundle "$BUNDLE_DIR" \
  --type "Quote" \
...
echo "--- create pitch ---"
pitch_line="$("$ROOT/virric/domains/research/scripts/create_product_pitch.sh" \
  --bundle "$BUNDLE_DIR" \
  --title "TEST Pitch" \
  | tail -n 1
)"
...
cpe_line="$("$ROOT/virric/domains/research/scripts/create_candidate_persona.sh" \
  --bundle "$BUNDLE_DIR" \
...
asm_line="$("$ROOT/virric/domains/research/scripts/assemble_research_assessment_bundle.sh" "$BUNDLE_DIR" | tail -n 1)"
