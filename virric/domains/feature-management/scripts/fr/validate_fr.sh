...
validate_status_value() {
  local file="$1"
  local status
  status="$(virric_fr_get_header "$file" "Status" 2>/dev/null || echo "")"
...
validate_priority_value() {
  local file="$1"
  local p
  p="$(virric_fr_get_header "$file" "Priority" 2>/dev/null || echo "")"
...
validate_id_matches_filename() {
  ...
  local fr_id
  fr_id="$(virric_fr_get_header "$file" "ID" 2>/dev/null || echo "")"
...
validate_content_not_empty() {
  ...
  for k in "${required[@]}"; do
    local v
    v="$(virric_fr_get_header "$file" "$k" 2>/dev/null | sed -e 's/^ *//; s/ *$//' || true)"
    if [[ -z "$v" ]]; then
