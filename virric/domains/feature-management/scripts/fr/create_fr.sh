...
if [[ -t 1 ]]; then
  VIRRIC_GREEN='\033[38;5;118m'
  VIRRIC_BRIGHT='\033[38;5;154m'
  CYAN_GLOW='\033[38;5;51m'
  YELLOW_HAZARD='\033[38;5;226m'
  MAGENTA_PULSE='\033[38;5;201m'
  RESET='\033[0m'
else
  VIRRIC_GREEN='' VIRRIC_BRIGHT='' CYAN_GLOW='' YELLOW_HAZARD='' MAGENTA_PULSE='' RESET=''
fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      echo -e "${VIRRIC_GREEN}[${YELLOW_HAZARD}ERROR${VIRRIC_GREEN}]${RESET} Unknown parameter: $1"
      exit 1
      ;;
  esac
done
...
if [[ -z "$title" || -z "$description" ]]; then
  echo -e "${VIRRIC_GREEN}[${YELLOW_HAZARD}ERROR${VIRRIC_GREEN}]${RESET} Title and description are required."
  exit 1
fi
...
echo -e "${VIRRIC_GREEN}[${MAGENTA_PULSE}INFO${VIRRIC_GREEN}]${RESET} Location: ${VIRRIC_BRIGHT}$fr_file${RESET}"
