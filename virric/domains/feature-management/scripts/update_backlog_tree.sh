#!/usr/bin/env bash
#
# Generates virric/domains/feature-management/docs/backlog_tree.md (bash-only)
...
header() {
  local timestamp="$1"
  cat <<EOF
# Project VIRRIC — Backlog Overview
...

  {
    ...
    for s in "${statuses[@]}"; do
      ...
      while IFS= read -r f; do
        [[ -z "$f" ]] && continue
        local status
        status="$(virric_fr_get_header "$f" "Status" 2>/dev/null || echo "")"
        if [[ "$status" == "$s" ]]; then
          local id name
          id="$(virric_fr_get_header "$f" "ID" 2>/dev/null || echo "")"
          name="$(virric_fr_get_header "$f" "Title" 2>/dev/null || echo "")"
