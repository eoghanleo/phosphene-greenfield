...
get_current_status() { virric_fr_get_header "$1" "Status" 2>/dev/null || echo ""; }
get_fr_title() { virric_fr_get_header "$1" "Title" 2>/dev/null || echo ""; }
...
update_fr_status_fields() {
  local fr_file="$1"
  local new_status="$2"
  virric_fr_set_header "$fr_file" "Status" "$new_status"
  virric_fr_set_header "$fr_file" "Updated" "$(date -u +"%Y-%m-%d")"
}
