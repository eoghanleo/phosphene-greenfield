...
get_fr_name() {
  virric_fr_get_header "$1" "Title" 2>/dev/null || echo "Unknown"
}

get_fr_status() {
  virric_fr_get_header "$1" "Status" 2>/dev/null || echo "Unknown"
}
...
find_fr_dependencies() {
  ...
  local deps
  deps="$(virric_fr_get_header "$fr_file" "Dependencies" 2>/dev/null || true)"
  if [[ -n "$deps" ]]; then
    echo "$deps" \
      | tr ',' '\n' \
      | sed -E 's/^ *//; s/ *$//' \
...
generate_tree() {
  ...
  {
    echo "# Project VIRRIC Feature Request Dependencies"
    echo ""
