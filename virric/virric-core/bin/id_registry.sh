...
usage() {
  cat <<'EOF'
Usage:
  ./virric/virric-core/bin/id_registry.sh [build|validate|where <ID>|next --type <type>]
...
build_index() {
  ...
  while IFS= read -r -d '' rel; do
    ...
    {
      git -C "$ROOT" ls-files -z -- "virric/domains"
      git -C "$ROOT" ls-files -z --others --exclude-standard -- "virric/domains"
    }
  )
