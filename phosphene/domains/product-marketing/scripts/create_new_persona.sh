...
#
# Usage:
#   ./phosphene/domains/product-marketing/scripts/create_new_persona.sh --title "Idle Ingrid" [--id PER-0003] [--owner "..."] [--status Draft] [--dependencies "CPE-0001,RA-001"] [--output-dir <dir>]
...
usage() {
  cat <<'EOF'
Usage:
  ./phosphene/domains/product-marketing/scripts/create_new_persona.sh --title "..." [--id PER-0001] [--owner "..."] [--status Draft] [--dependencies "..."] [--output-dir <dir>]
EOF
}
...
STATUS="Draft"
DEPENDENCIES=""
OUT_DIR="$ROOT/phosphene/domains/product-marketing/docs/personas"
...
TEMPLATE="$ROOT/phosphene/domains/product-marketing/templates/persona.md"
...
set +e
"$ROOT/phosphene/domains/product-marketing/scripts/validate_persona.sh" --strict "$OUT" >/dev/null
rc=$?
set -e
if [[ $rc -ne 0 ]]; then
  rm -f "$OUT" || true
