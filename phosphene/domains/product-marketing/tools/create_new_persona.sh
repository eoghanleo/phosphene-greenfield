...
#
# Usage:
#   ./phosphene/domains/product-marketing/tools/create_new_persona.sh --title "Idle Ingrid" --vpd VPD-001 [--id PER-0003] [--owner "..."] [--status Draft] [--dependencies "CPE-0001,RA-001"] [--output-dir <dir>]
...
usage() {
  cat <<'EOF'
Usage:
  ./phosphene/domains/product-marketing/tools/create_new_persona.sh --title "..." --vpd VPD-001 [--id PER-0001] [--owner "..."] [--status Draft] [--dependencies "..."] [--output-dir <dir>]
EOF
}
...
set +e
"$ROOT/phosphene/domains/product-marketing/tools/validate_persona.sh" --strict "$TMP_EDIT" >/dev/null
rc=$?
set -e
if [[ $rc -ne 0 ]]; then
