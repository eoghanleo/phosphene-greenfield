...
#
# Usage:
#   ./phosphene/domains/product-marketing/tools/add_proposition_pain_reliever.sh --proposition <file> --reliever "..." [--mapped-pains "JTBD-PAIN-0001-PER-0001,JTBD-PAIN-0002-PER-0001"]
...
usage() {
  cat <<'EOF'
Usage:
  ./phosphene/domains/product-marketing/tools/add_proposition_pain_reliever.sh --proposition <file> --reliever "..." [--mapped-pains "..."]
EOF
}
...
if ! "$ROOT/phosphene/domains/product-marketing/tools/validate_proposition.sh" "$TMP_OUT" >/dev/null; then
  rc=$?
  rm -f "$TMP_OUT" || true
  exit $rc
fi
...
"$ROOT/phosphene/domains/product-marketing/tools/validate_proposition.sh" "$PROP" >/dev/null
